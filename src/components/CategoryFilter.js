...
export default function CategoryFilter({ selected, onSelect }) {
    return (_jsx("div", { className: "flex flex-wrap gap-1.5 sm:gap-2", children: categories.map((cat) => (_jsxs("button", { onClick: () => onSelect(cat), className: `relative px-3 sm:px-5 py-1.5 sm:py-2 rounded-full text-xs sm:text-sm font-medium transition-colors ${selected === cat
